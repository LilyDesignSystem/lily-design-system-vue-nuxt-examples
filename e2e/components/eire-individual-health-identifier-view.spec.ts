...
test.describe('component page: eire-individual-health-identifier-view', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/eire-individual-health-identifier-view');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/eire-individual-health-identifier-view');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^EireIndividualHealthIdentifierView$') })
    ).toBeVisible();
  });
