...
test.describe('component page: tel-input', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/tel-input');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/tel-input');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^TelInput$') })
    ).toBeVisible();
  });
