...
test.describe('component page: gantt-table-td', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/gantt-table-td');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/gantt-table-td');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^GanttTableTD$') })
    ).toBeVisible();
  });
