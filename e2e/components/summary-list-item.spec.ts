...
test.describe('component page: summary-list-item', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/summary-list-item');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/summary-list-item');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^SummaryListItem$') })
    ).toBeVisible();
  });
