...
test.describe('component page: editable-form', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/editable-form');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/editable-form');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^EditableForm$') })
    ).toBeVisible();
  });
