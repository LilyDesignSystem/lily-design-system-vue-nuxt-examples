...
test.describe('component page: grail-layout-top-header', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/grail-layout-top-header');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/grail-layout-top-header');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^GrailLayoutTopHeader$') })
    ).toBeVisible();
  });
