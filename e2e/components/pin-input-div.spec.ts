...
test.describe('component page: pin-input-div', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/pin-input-div');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/pin-input-div');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^PinInputDiv$') })
    ).toBeVisible();
  });
