...
test.describe('component page: mockup-phone-portrait', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/mockup-phone-portrait');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/mockup-phone-portrait');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^MockupPhonePortrait$') })
    ).toBeVisible();
  });
