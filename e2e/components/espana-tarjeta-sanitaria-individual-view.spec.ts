...
test.describe('component page: espana-tarjeta-sanitaria-individual-view', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/espana-tarjeta-sanitaria-individual-view');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/espana-tarjeta-sanitaria-individual-view');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^EspanaTarjetaSanitariaIndividualView$') })
    ).toBeVisible();
  });
