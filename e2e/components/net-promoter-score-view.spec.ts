...
test.describe('component page: net-promoter-score-view', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/net-promoter-score-view');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/net-promoter-score-view');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^NetPromoterScoreView$') })
    ).toBeVisible();
  });
