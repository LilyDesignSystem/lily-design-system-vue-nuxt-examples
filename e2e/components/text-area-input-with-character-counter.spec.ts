...
test.describe('component page: text-area-input-with-character-counter', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/text-area-input-with-character-counter');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/text-area-input-with-character-counter');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^TextAreaInputWithCharacterCounter$') })
    ).toBeVisible();
  });
