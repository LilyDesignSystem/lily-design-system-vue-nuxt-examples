...
test.describe('component page: kanban-table-foot', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/kanban-table-foot');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/kanban-table-foot');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^KanbanTableFoot$') })
    ).toBeVisible();
  });
