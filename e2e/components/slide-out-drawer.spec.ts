...
test.describe('component page: slide-out-drawer', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/components/slide-out-drawer');
  });

  test('responds with a non-error status', async ({ page }) => {
    const res = await page.goto('/components/slide-out-drawer');
  ...
  test('renders the H1 with the component name', async ({ page }) => {
    await expect(
      page.getByRole('heading', { level: 1, name: new RegExp('^SlideOutDrawer$') })
    ).toBeVisible();
  });
